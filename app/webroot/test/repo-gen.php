...
{
    $file = apk_info($APK);
//    exec('soft\\unzip -p "' . $APK . '" ' . $info['icon'] . ' > iconslaunchers/' . $info['id'] . '.png');
    ...
    $path .= $file['id'] . '.png';
    exec('soft\\unzip -p "' . $APK . '" ' . $file['icon'] . ' > ' . $path);
}
...
function createdb()
{
    $info = apk_info('SAS.apk');
    $jaas_description = "Aplicacion para consultar la BD de aplicaciones de android de la Cujae!";
    $jass_category = "Herramientas";
    $db = db();
...
function repo_clean(){
    ...
    while ($row = $apks->fetch(\PDO::FETCH_ASSOC)) {
        ...
        unlink('iconslaunchers/'.$row['id'].'.png');
...
function repo_gen($params)
{
    if (isset($params['dir'])) {
        ...
        //if (!is_dir('iconslaunchers')) {
        //    mkdir('iconslaunchers', 0777);
        //}
        copy_to_pool('SAS.apk');
        apk_icon('SAS.apk');
